...
app.get('/app', (req, res) => {
  ...
  // Inject upload JS functions if missing from app.js
  if (!html.includes('function processDocument')) {
    const docJs = '<script>var actionLabels={summarize:"Summarize",\'fix-errors\':"Fix Errors",\'translate-zh2en\':"Chinese\\u2192English",\'translate-en2zh\':"English\\u2192Chinese",\'business-translate\':"Business Translate"};function handleDocUpload(i){var f=i.files?i.files[0]:i;if(!f)return;document.getElementById("docUploadPlaceholder").style.display="none";document.getElementById("docUploadFileInfo").style.display="flex";document.getElementById("docFileName").textContent=f.name+" ("+(f.size/1024).toFixed(1)+" KB)";document.getElementById("docResultArea").style.display="none";document.getElementById("docResultArea").innerHTML="";toast("File loaded: "+f.name,"var(--gold)",2000)}function clearDocUpload(){document.getElementById("docFileInput").value="";document.getElementById("docUploadPlaceholder").style.display="block";document.getElementById("docUploadFileInfo").style.display="none";document.getElementById("docFileName").textContent="";document.getElementById("docResultArea").style.display="none";document.getElementById("docResultArea").innerHTML=""}function processDocument(a){var i=document.getElementById("docFileInput"),f=i.files[0];if(!f)return toast("Please select a file first","var(--accent)");if(localStorage.getItem("is_premium")!=="true")return showPremiumPaywall("Document Upload");var r=document.getElementById("docResultArea");r.style.display="block";r.innerHTML="<div class=\\"text-center py-4\\"><i class=\\"fas fa-spinner fa-spin text-lg\\" style=\\"color:var(--gold)\\"></i><div class=\\"mt-1\\" style=\\"color:var(--muted)\\">Processing document...</div></div>";document.getElementById("docActions").style.display="none";var imgExts=[".jpg",".jpeg",".png",".webp",".bmp",".heic",".heif"];if(f.type.startsWith("image/")||imgExts.some(function(e){return f.name.toLowerCase().endsWith(e)})){var img=new Image();img.onload=function(){var c=document.createElement("canvas"),w=img.width,h=img.height,m=200;if(w>h&&w>m){h=h*m/w;w=m}else if(h>m){w=w*m/h;h=m}c.width=w;c.height=h;c.getContext("2d").drawImage(img,0,0,w,h);c.toBlob(function(b){if(!b){sendDoc(f,a,r);return}sendDoc(new File([b],f.name,{type:"image/jpeg"}),a,r)},"image/jpeg",0.5)};img.onerror=function(){sendDoc(f,a,r)};img.src=URL.createObjectURL(f)}else sendDoc(f,a,r)}function sendDoc(f,a,r){var fd=new FormData();fd.append("document",f);fd.append("action",a);fetch("/api/upload-document",{method:"POST",body:fd}).then(function(x){return x.json()}).then(function(d){if(d.error){var m=typeof d.error==="object"?(d.error.error||JSON.stringify(d.error)):d.error;r.innerHTML="<div style=\\"color:var(--accent)\\" class=\\"py-2\\"><b>Error:</b> "+escapeHtml(m.slice(0,500))+"</div>";return}r.innerHTML="<div class=\\"font-bold mb-1\\" style=\\"color:var(--gold)\\">"+actionLabels[a]+"</div><div style=\\"color:var(--fg);white-space:pre-wrap\\">"+escapeHtml(d.response)+"</div>"})["catch"](function(e){r.innerHTML="<div style=\\"color:var(--accent)\\" class=\\"py-2\\"><b>Upload failed:</b> "+escapeHtml(e.message)+"</div>"})}</script>';
    const bodyEnd = html.indexOf('</body>');
    if (bodyEnd !== -1) html = html.slice(0, bodyEnd) + docJs + '\n' + html.slice(bodyEnd);
  }
  res.send(html);
});
